...
class GroupPostViewMapper(
    ...
    private val resourceRevisionViewMapper: ResourceRevisionViewMapper,
    private val groupViewMapper: GroupViewMapper,
    private val resourceViewMapper: ResourceViewMapper
) {

    fun mapView(
        entity: GroupPostEntity,
        userId: UserId
    ) = GroupPostView(
        id = entity.id,
        group = groupViewMapper.mapMinimalView(entity.groupId),
        resource = resourceViewMapper.mapView(entity.resourceId),
        myRevision = resourceRevisionViewMapper.mapView(
            resourceRevisionRepository.findUserSavedRevision(
                groupPostId = entity.id,
                userId = userId
            )
        )
    )
